...
class Young_Moudulous_Map(torch.nn.Module):
    ...
    def __init__(self, E_item, inverted_index, gs_num, device):
        super(Young_Moudulous_Map, self).__init__()
        self.E = torch.nn.Parameter(E_item.to(device))
        self.inverted_index = inverted_index.to(device)
        self.gs_num = gs_num

    def forward(self):
        E_out = self.E[self.inverted_index]
        return E_out
